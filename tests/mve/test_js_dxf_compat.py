"""ブラウザ版DXF書き出し（web/mve/dxf.js）がJW-CAD（JWW）で読めることの検証.

Python版の検証（test_jww_dxf_compat.py）と同じ観点を、Node.jsで実際に
web/mve/dxf.jsを走らせた出力に対して確認する。CP932への変換は
...
def test_cp932_table_matches_python_codec():
    ...
    table_path = Path(__file__).resolve().parents[2] / "web" / "mve" / "cp932_table.js"

...
この場合、**行政GIS（用途地域・地区計画・防火地域・高度地区・地価公示）は
使えません**。reinfolibはCORS制限のためブラウザから直接取得できないためです。
用途地域などをSTEP2に手入力すればGeoJSONは作れます。

### Electronで開く（行政GISも使う・推奨）
...
別の場所にある `index.html` を開くこともできます。

```bash
SITEINFO_HTML="D:\\GD\\[Claude]\\SIM_engines\\siteinfo\\index.html" npm start
npm start -- "D:\\path\\to\\index.html"
```

APIキーは[不動産情報ライブラリ](https://www.reinfolib.mlit.go.jp/)で無償申請できます。
STEP2の入力欄に直接入れることもできます。

## 操作の流れ

1. **STEP1 位置と形状** — 住所検索／地図に描画／地点情報モード／GeoJSON読み込みの
   4通り（併用可）。3点以上で確定すると面積が出ます。確定測量図の値は
   「面積の手入力」に入れてください（`area_is_manual`に記録されます）。
2. **STEP2 敷地条件・行政情報** — 「🏛 自動入力」で用途地域・容積率・建ぺい率・
   地区計画・防火地域・高度地区・地価公示を取得します。
   **道路幅員と日影規制は全国統一のAPIが無いため常に手入力**です。
3. **STEP3 書き出し** — 画面右の「取得した敷地情報」が、そのまま書き出される内容です。
...
### 行政GISが取れない（`window.gisAPI が未定義`）

Electronホスト経由で起動していません。`cd siteinfo/electron && npm start` で
起動してください。独自のElectronホストを使う場合は、preloadで
`contextBridge.exposeInMainWorld("gisAPI", { fetch })` を公開する必要があります。

かつては `Cannot read properties of undefined (reading 'fetch')` という
TypeErrorが4レイヤー分出ていましたが、現在は原因を示して中断します。
...
| 表示 | 原因と対処 |
|---|---|
| `CesiumJSの読み込みに失敗しました（…）` | CDNに到達できない（cesium.com → unpkg → jsDelivr の順に試します） |

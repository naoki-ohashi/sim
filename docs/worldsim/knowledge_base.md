...
Obsidian のコアプラグイン「テンプレート」で、frontmatter と節立てのひな形を挿入できます。
テンプレートはリポジトリ直下の `templates/` に置き、`.obsidian/templates.json` で
フォルダと日付形式（`YYYY-MM-DD`）を設定してあります。`docs/` の外に置くのは、
索引やダッシュボードにひな形が載らないようにするためです。

| テンプレート | 使う場面 | 入るもの |
|---|---|---|
| [`受信箱メモ`](../../templates/受信箱メモ.md) | `docs/inbox/<tool>/` に新しい md を作る | `status: inbox`、提案・根拠・正本への反映案 |
| [`正本`](../../templates/正本.md) | 新しい正本を作る（1 テーマ 1 ファイル） | `status: draft`、目的と範囲・本文・未決・出典 |
| [`未決・決定`](../../templates/未決・決定.md) | 正本の中に未決事項を 1 件足す | 選択肢・論点・出典（決まったら「決定（日付）」に変える） |
| [`統合済み`](../../templates/統合済み.md) | 受信箱の md を `docs/archive/` へ移したとき | `status: archived` と「→ 統合先:」の行 |

使い方（初回だけ、PC ごと）: 設定 → コアプラグイン → 「テンプレート」をオンにする。
フォルダはコミット済みの設定で `templates` になっている。以後は新しいノートを作り、
コマンドパレット →「テンプレート: テンプレートを挿入」で選ぶ。`{{title}}` はファイル名、
`{{date:YYYY-MM-DD}}` は挿入した日に置き換わる。

- 既に frontmatter があるノートに挿入すると、プロパティが統合される。
  `統合済み` を挿入したあとは `status: archived` になっているか確認する。
- エージェントは Obsidian を使わないので、同じ節立てで直接書く
  （`{{...}}` は実際のファイル名・日付に置き換える）。
- `templates/` の md は索引（`docs/INDEX.md`）には載らない。

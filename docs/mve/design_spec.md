...
| 検証 | 自動テスト430件（うちMVE 266件）、DXF出力は実機JW-CADで表示確認済み |
...
4. **出力**: DXFではレベルごとに `MVE-ISOCHRONE-{時間}H` レイヤに
   ...
   表示する（`viewer3d.py`）。ブラウザ版にも `web/mve/isochrone.js` として
...
ブラウザ版（`web/mve/engine.js`・`optimizer.js`）も同じロジックで天空率に
対応しています（Python版の `sky_ratio.py`・`sky_index.py`・`optimizer.py`
の該当部分を1:1で移植、`tests/mve/test_js_parity.py` で日影との同時最適化を
...
| `MVE-ISOCHRONE-{時間}H` | 等時間日影線（`isochrone_hours` 指定時のみ、5.3.1節） |
| `MVE-NORTH` | 真北記号 |
...
| `test_js_dxf_compat.py` | 10 | **ブラウザ版のDXF書き出し（`web/mve/dxf.js`）がJW-CADで読める形式かをNode.jsで検証** |
| `test_joint_optimization.py` | 9 | **日影と天空率の同時最適化。voxelの交互解消・逆日影の同時探索の両方を検証** |
| `test_north.py` | 8 | 真北の方位角変換 |
| `test_js_site_import_parity.py` | 7 | **敷地JSON/CSV読み込み（`web/mve/site_import.js`）をPython版と突き合わせ** |

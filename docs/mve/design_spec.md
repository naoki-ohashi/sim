...
| 規模 | Python 4,350行 / 28モジュール、JavaScript 1,380行 |
| 検証 | 自動テスト358件（うちMVE 192件）、DXF出力は実機JW-CADで表示確認済み |
...
積み直し（5.6節）は行いません。個別に積み直すと、せっかくの規則正しい
形が崩れてしまうためです。
...
3. **Windows exe の実機検証**
4. **逆日影と天空率の同時最適化** — 現状は日影（自由形または屋根形状）→
   天空率の順に別々に解決する。両方を同時に動かす1本の探索にできれば、
   さらに容積を伸ばせる可能性がある

...
| 規模 | Python 3,900行 / 26モジュール、JavaScript 1,380行 |
| 検証 | 自動テスト334件（うちMVE 168件）、DXF出力は実機JW-CADで表示確認済み |
...
- **天空率の絶対値の算定** — 適合判定は行いますが（5.4節）、告示が定める
...
`use_sky_ratio: true` のときだけ動きます。斜線制限を外した代わりに、
**すべての測定点で Ps ≧ Pr を満たすまで下げます**。
...
mesh:
  ...
  # use_sky_ratio: false      # true で斜線制限に代えて天空率で判定（5.4節）
...
| `test_js_parity.py` | 17 | **Node.jsでJS版を実行しPython版と突き合わせ** |

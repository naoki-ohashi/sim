...
| 規模 | Python 3,558行 / 25モジュール、JavaScript 1,357行 |
| 検証 | 自動テスト318件（うちMVE 152件）、DXF出力は実機JW-CADで表示確認済み |
...
- **天空率の絶対値の算定** — 告示が定める厳密な投影法に準拠していません
...
mve/
  ...
  shadow_index.py    しきい値高さインデックス ★中核
  ...
  regulations/
    ...
    sky_ratio.py       天空率（法56条7項）※最適化には未接続（6.3節）
  io/
...
OptimizeResult(site, area, floors, blocks, far, shadow_spec, shadow_lines,
               coverage_limited, far_limited, shadow_limited,
               volume_removed_by_shadow_m3, notes)
```
...
### 6.3 天空率が最適化に接続されていない

`use_sky_ratio: true` は現状、**斜線制限を外して絶対高さ制限だけを見る**
動作です。`sky_ratio.check()` による適合判定は最適化パイプラインから
呼ばれていません。

つまりこのオプションは「天空率で斜線制限に代えられる前提で、上限がどこまで
上がるか」を見るためのものであり、**天空率の適合を保証しません**。実際に
使う場合は、得られた形状を別途 `sky_ratio.check()` で確認する必要があります。

天空率エンジン自体（`sky_ratio.py`）は Ps/Pr の比較として実装済みで、
適合建築物の生成（`reference_building`）と併せて3Dビューアの比較表示には
使われています。**最適化ループへの接続は今後の課題です。**
...
mesh:
  ...
  # use_sky_ratio: false      # 6.3節の制約に注意
  # max_iterations: 4000
...
| `test_optimizer.py` | 20 | 貪欲最適化。旧方式との比較を含む |
| `test_shadow.py` | 20 | 日影時間・みなし境界線・測定面 |
| `test_io.py` | 19 | DXF入出力・3D・設定読み込み |
| `test_js_parity.py` | 15 | **Node.jsでJS版を実行しPython版と突き合わせ** |
...
1. **天空率を最適化ループへ接続する**（6.3節）— 現状は上限を外すだけ
2. **JWW外部変形をMVEへ載せ替える** — `jwcad_volume` が旧エンジンのまま
3. **Windows exe の実機検証**
4. 令135条の12第2項（日影の高低差緩和）
5. 法52条9項（特定道路による緩和）
6. 平均地盤面の算定（傾斜地対応）

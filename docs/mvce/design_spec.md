...
**MVCE（Maximum Volume Engine）** — 日影規制・斜線制限・容積率をふまえて、
敷地に建てられる最大容積を求める計算エンジン。

| | |
|---|---|
| 版 | 2026-08 時点 |
| 対象 | `mvce/` パッケージ（Python 3.9+）および `web/mvce/`（ブラウザ版） |
| 規模 | Python 5,409行 / 28モジュール、JavaScript 2,745行 |
| 検証 | 自動テスト430件（うちMVCE 266件）、DXF出力は実機JW-CADで表示確認済み |
...
容積を大きく取り逃します。MVCEは敷地をメッシュに割り、**その測定点を実際に
日影にしているマスだけ**を特定して下げます。結果として、日影に効く方向の
マスだけが段状に低くなり、他は上限のまま残ります。

同一条件での比較（`tests/mvce/test_optimizer.py`）:

| 方式 | 延床面積 |
|---|---|
| 一律に低くする（旧方式） | 478 m² |
| ボクセル法（MVCE、自由形） | **1,144 m²** |
...
- **等時間日影線の告示水準の精度** — 作図はできますが（5.3節）、グリッド
  サンプリングによる近似です
...
担保**しています（`tests/mvce/test_js_parity.py`、Node.jsで実際にJS版を走らせて
Python版と突き合わせ）。

| | Python版 | ブラウザ版 |
|---|---|---|
| 実体 | `mvce/` | `web/mvce/`（単一HTMLに結合可） |
| 必要なもの | Python + shapely/numpy/ezdxf/PyYAML | ブラウザのみ |
| 入力 | YAML・DXF敷地図 | フォーム・設定YAML・敷地JSON/CSV |
| 出力 | DXF図面・3D HTML・サマリー | 平面図・3D・設定YAML・DXF図面 |
| DXF読込 | あり | **なし** |
| DXF書出 | あり | あり |
| 天空率（法56条7項） | あり | あり（同じロジックで移植済み） |
| 等時間日影線 | あり | あり |
| 逆日影（lean_to / ridge） | あり | **なし**（指定するとエラーになる） |
...
mvce/
  ...
  isochrone.py       等時間日影線（グリッド生成・マーチングスクエア法）
  ...
  regulations/
    ...
    sky_ratio.py       天空率（法56条7項、Ps/Pr の算定。令135条の5〜11）
  io/
    dxf_site.py        DXF敷地図の読み込み
    site_json.py       敷地JSON読み込み
    site_csv.py        敷地CSV読み込み
...
`shadow.isochrone_hours` に時間のリスト（例 `[2.0, 3.0, 4.0, 5.0]`）を
指定すると、2時間線・3時間線…の等時間日影線を追加で作図します。

1. **グリッド生成**（`isochrone.py` の `_default_grid_margin_m`）:
   敷地の外接矩形を、既定では建物の最高高さと冬至の最低太陽高度から
   `reach = (最高高さ − 測定面高さ) / tan(最低太陽高度)` を求めて
   その分だけ広げる（`isochrone_margin_m` を指定すれば固定値も可）。
   間隔は既定2.0m（`isochrone_grid_interval_m`）。
2. **各グリッド点の日影時間**（`shadow_index.grid_shadow_hours`）:
   5.3節の「しきい値高さ」と同じ仕組みを、5m/10m測定点専用の配列では
   なく任意の点集合に対して使えるよう一般化したもの。建物内部の点も
   特別扱いせず同じ式で計算する（しきい値式は r≈0 でも自然に成立し、
   建物際で等高線が自然に閉じるため、人為的なマスク処理は入れていない）。
3. **等高線抽出**（`isochrone.compute_isochrones`）:
   外部ライブラリ（scipy/skimage）を使わない自前のマーチングスクエア法。
   各セルの4隅とレベルの大小関係（16通り）から交差線分を線形補間で
   求め、鞍点（対角だけが高い/低いケース）はセル中心値との比較で
...
4. **出力**: DXFではレベルごとに `MVCE-ISOCHRONE-{時間}H` レイヤに
   ...
   表示する（`viewer3d.py`）。ブラウザ版にも `web/mvce/isochrone.js` として
   1:1で移植済みで、3Dタブに同じ等高線を重畳表示します。
...
考え方です（`tests/mvce/test_roof_envelope.py`）。
...
適合建築物は**3つの高さ制限すべての共通部分ではなく、道路・隣地・北側
それぞれ独立**です（`reference_buildings`）。道路用の適合建築物は隣地斜線・
北側斜線を考慮せず、隣地用・北側用も同様に他の2つを考慮しません。測定点も
`kind`（road / adjacent / north）で区分されているため、道路の測定点は道路用の
適合建築物と、隣地の測定点は隣地用の適合建築物と、北側の測定点は北側用の
適合建築物と比較します。

```python
_required_setback_for_kind(site, edge_index, height_m, kind)
    # road:     道路斜線の後退距離（road_slant）。道路以外の辺は0（無制限）
    # adjacent: 隣地斜線の後退距離（adjacent_slant）。隣地以外の辺は0
    # north:    北側斜線の後退距離（north_slant）。北側を向かない辺は0

reference_building_for_kind(site, kind)
    # 上の後退距離だけで作った buildable_ring を階段状に積んだもの
```

3区分をまとめた `reference_building`（統合エンベロープ）は3Dビューアの表示
専用で残しています。他の区分に制約されないぶん、区分ごとの適合建築物は
統合エンベロープより高くなることがあります（`_max_height_for_kind`）。
...
両方向の不等式が成り立つため一致します。`tests/mvce/test_sky_ratio.py` が
...
**トレードオフの実例**（`tests/mvce/test_joint_optimization.py`）: ある
...
MVCEは同時探索が成功する限りそちらを採用します。屋根が規則正しい1〜2枚の
...
- 等時間日影線はグリッドサンプリング＋マーチングスクエア法による近似
  （`isochrone_hours` を指定したときのみ作図、5.3.1節）。ブラウザ版
  UIにも移植済み

### 6.3 天空率の適合は保証されるが、絶対値は認定ソフトと異なる

`use_sky_ratio: true` は斜線制限を外し、**代わりに全測定点で Ps ≧ Pr を確認
して満たすまで下げます**（5.5節）。Pr は道路・隣地・北側で別々の適合建築物
から求め、各測定点は対応する区分の適合建築物とだけ比較します（令135条の
5〜7）。適合しない形状が結果として返ることはありません。適合しきれない
場合は `sky_ratio_ok` が偽になり、サマリーと `notes` に理由が出ます。
...
ブラウザ版（`web/mvce/engine.js`・`optimizer.js`）も同じロジックで天空率に
対応しています（Python版の `sky_ratio.py`・`sky_index.py`・`optimizer.py`
の該当部分を1:1で移植、`tests/mvce/test_js_parity.py` で日影との同時最適化を
含めて数値一致を検証）。`envelope_family: lean_to | ridge`（逆日影の屋根越し
・棟状パターン、5.4節）はブラウザ版に一度も移植されていないスコープ外機能で、
天空率を使う場合もブラウザ版は自由形（voxel）のみに対応します。
...
| `MVCE-ISOCHRONE-{時間}H` | 等時間日影線（`isochrone_hours` 指定時のみ、5.3.1節） |
| `MVCE-NORTH` | 真北記号 |
...
Windowsでは `MVCE実行.bat` のダブルクリックで、Python検出・依存導入・
...
| 対象 | 件数 | 内容 |
|---|---|---|
| `test_slant_lines.py` | 31 | 3斜線と全緩和・道路の反対側境界線。法令の基準値で検証 |
| `test_io.py` | 29 | DXF入出力（棟・等時間日影線の描画を含む）・3D・設定読み込み |
| `test_js_parity.py` | 28 | **Node.jsでJS版を実行しPython版と突き合わせ（天空率・日影との同時最適化を含む）** |
| `test_optimizer.py` | 25 | 貪欲最適化。旧方式・逆日影との比較を含む |
| `test_shadow.py` | 20 | 日影時間・みなし境界線・測定面 |
| `test_site_import.py` | 19 | **敷地図のJSON/CSV読み込み** |
| `test_jww_dxf_compat.py` | 15 | DXF書式の固定（再発防止） |
| `test_isochrone.py` | 15 | **等時間日影図（マーチングスクエア法・グリッド計算）** |
| `test_sky_ratio.py` | 15 | 天空率の接続。独立計算での適合検証、**道路・隣地・北側の適合建築物が別々であることの検証を含む** |
| `test_roof_envelope.py` | 12 | **逆日影（屋根越し・棟状パターン）。独立計算での適合検証を含む** |
| `test_web_ui.py` | 11 | Chromiumで実際に操作（天空率チェックボックスを含む） |
| `test_js_dxf_compat.py` | 10 | **ブラウザ版のDXF書き出し（`web/mvce/dxf.js`）がJW-CADで読める形式かをNode.jsで検証** |
| `test_joint_optimization.py` | 9 | **日影と天空率の同時最適化。voxelの交互解消・逆日影の同時探索の両方を検証** |
| `test_north.py` | 8 | 真北の方位角変換 |
| `test_js_site_import_parity.py` | 7 | **敷地JSON/CSV読み込み（`web/mvce/site_import.js`）をPython版と突き合わせ** |
| `test_far.py` | 7 | 法52条2項 |
| `test_cli_output_paths.py` | 5 | 出力先・エラー処理 |
...
みなし境界線・メッシュ・**各マスの階数**・延床面積・天空率（方位・測定点・
適合建築物・Ps/Pr・単独最適化・**日影との同時最適化**）・サマリー文字列まで
突き合わせます。
...
1. **逆日影（lean_to/ridge）をブラウザ版へ移植する** — 現在はPython版のみ
   （天空率・等時間日影線・敷地JSON/CSV読み込みは移植済み、6.3節参照）
2. **JWW外部変形をMVCEへ載せ替える** — `jwcad_volume` が旧エンジンのまま
3. **Windows exe の実機検証**
...
実装済み。等時間日影線・敷地JSON/CSV読み込み・DXF書き出しのブラウザ版
移植（6.3節）。

...
- **等時間日影線の告示水準の精度** — 作図はできますが（5.3節）、グリッド
  サンプリングによる近似です
- **平均地盤面の算定**（令2条2項） — `ground.py` で算定できますが、高さの判定はまだ Z=0 から測っています
...
mvce/
  ...
  verdict.py         判定の型と UNDETERMINED の伝播
  index/
    shadow_index.py    日影のしきい値高さインデックス ★中核
    sky_index.py       天空率の入射距離インデックス   ★中核
    isochrone.py       等時間日影線（マーチングスクエア法）
  inverse/
    shadow_envelope.py 逆日影（屋根越し・棟状パターン） ★中核
  solvers/
    optimizer.py       ボクセル貪欲最適化             ★中核
  profiles/          審査機関別プロファイル（Phase 2 で実装）
  regulations/
...
辺ごとに `specified_road: {width_m, distance_m}` を入れると効きます。
敷地ポリゴンからは導けない情報（特定道路がどこにあるか）なので、
都市計画図・道路台帳で調べた値を入力してもらいます。

**読み替えの範囲は「第二項から第七項まで」に限られます。** 斜線制限
（令132条・令134条）の幅員は実幅員のまま。`site.max_road_width_m` が実幅員、
`far.far_road_width_m()` が読み替え後で、別々の関数に分けてあります。

> **未対応:** 特定行政庁が定める割増（法52条2項各号の指定区域）、
> 法52条8項（住宅の容積率の割増）、法52条10項〜14項（計画道路・壁面線・
> 特定行政庁の許可）。該当し得る場合は `notes` に注意書きを出します。
...
> **対応済み（2026-08-30）:** 高低差緩和は**令135条の12第3項第2号**です
> （第2項ではありません。照合台帳の食い違い A）。敷地の平均地盤面が隣地等の
> 地盤面より1m以上低い場合、平均地盤面が `(高低差−1)/2` だけ高い位置に
> あるものとみなし、そのぶん測定面が上がります。`site.shadow_ground` で
> 隣地等の地盤面を入力します（既定は緩和なし）。第4項の特定行政庁の定めも
> `designated_level_m` で受けられます。
...
`index/shadow_index.py` が持つ**しきい値高さ**が、この処理を可能にします。
...
1. **グリッド生成**（`index/isochrone.py` の `_default_grid_margin_m`）:
...
`envelope_family: "lean_to" | "ridge"` を指定したときの別解です（`inverse/shadow_envelope.py`）。
...
考え方です（`tests/mvce/test_shadow_envelope.py`）。
...
`index/sky_index.py` は日影と同じ形のインデックスを持ちます。測定点 P、方位 φ、
...
`solvers/optimizer.py` 側が自由形の `_resolve_sky_ratio` で別途行います。
...
- 平均地盤面（令2条2項）は `ground.py` で算定できるが、斜線・絶対高さ・日影の測定面をそこから測るところまで繋いでいない（傾斜地で乖離。地盤の高さを入れると注記が出る）
- 等時間日影線はグリッドサンプリング＋マーチングスクエア法による近似
  （`isochrone_hours` を指定したときのみ作図、5.3.1節）。JS/ブラウザ版
  UIへの移植は対象外
...
対応しています（Python版の `sky_ratio.py`・`index/sky_index.py`・`solvers/optimizer.py`
の該当部分を1:1で移植、`tests/mvce/test_js_parity.py` で日影との同時最適化を
含めて数値一致を検証）。

**前面道路が2以上ある敷地の天空率は Python 版だけです**（2026-08-31）。
令135条の6第3項・令135条の9第3項が求める令132条の区域ごとの比較は、多角形の
差集合・和集合が要ります。ブラウザ版は半平面クリップしか持たないので実装
できず、従来どおりエラーで止まります（止まる側なので危険ではありません）。`envelope_family: lean_to | ridge`（逆日影の屋根越し
...
| 条文 | 内容 |
|---|---|
| 令2条2項 | 平均地盤面から高さを測るところまでの接続（算定自体は実装済み） |
| 令132条 | 3本以上が複雑に絡む場合の運用差 |
| 令134条2項 | 天空率の区域を多角形で作れない（点ごとの斜線判定は実装済み）。<br>`apply_article_134_2` かつ前面道路の反対側に公園等がある敷地で<br>天空率を使うと `UndeterminedRegulation` |
...
| `test_js_parity.py` | 30 | **Node.jsでJS版を実行しPython版と突き合わせ（天空率・日影との同時最適化を含む）** |
...
| `test_shadow_envelope.py` | 13 | **逆日影（屋根越し・棟状パターン）。独立計算での適合検証を含む** |
...
1. **逆日影（lean_to/ridge）をブラウザ版へ移植する** — 現在はPython版のみ
   （天空率は移植済み、6.3節参照）
2. **等時間日影線をブラウザ版へ移植する** — 現在はPython版のみ（DXF・
   Python生成の3D HTMLに出力、5.3.1節）
3. **JWW外部変形をMVEへ載せ替える** — `jwcad_volume` が旧エンジンのまま
4. **Windows exe の実機検証**
5. **実務者のノウハウ・過去のボリュームチェック図面を学習するモジュール**
   （次段階の構想として合意済み。まだ設計に着手していない）
6. 平均地盤面から高さを測るところまでの接続（傾斜地対応）

...
(function (global) {
  ...
  // 前面道路が2以上なら令135条の6第3項の区域ごとの比較が要る。区域区分には
  // 多角形の差集合・和集合が要り、こちらは半平面クリップしか持たないので
  // 実装できない → throw（止まる側なので危険ではない）。Python 版は
  // regulations/road_regions.py で実装済み（2026-08-31）。
  function skyRoadBand(site) {
    ...
    if (roads.length >= 2) {
      throw new Error(
        ...
        + '求めています。ブラウザ版は多角形の集合演算を持たず区域分割ができない'
        + 'ため、天空率による道路高さ制限の適用除外（法56条7項1号）は判定'
        + 'できません。Python 版なら判定できます。ブラウザ版で続けるなら'

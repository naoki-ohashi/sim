...
`statutes/建築基準法.md` に収録した法58条は**平成30年版**で、現行版では
ありません（照合台帳の食い違い E）。令和7年12月1日施行の施行令
第2条第1項第六号ロが「法第五十八条第一項及び第二項」と参照しているので、
現行法には**第2項**があるはずですが、原文を取得できていません。

第2項は特定行政庁の許可に関する規定と推測されますが、**推測では実装
しません**。許可はいずれにせよ個別判断なので、エンジンが計算する話では
ないと考えています。原文が手に入ったら確認してください。
"""

...
## 測定面（法56条の2第1項・別表第四（は）欄）

> 従来この docstring は根拠を「令135条の12第1項」としていましたが誤り
> でした。同項は許可に係る「位置」の規定で、測定面とは関係ありません。
> 正しい根拠は法56条の2第1項が参照する**別表第四（は）欄**です
> （`docs/mvce/legal_basis.md` の「食い違い B」）。

平均地盤面からの高さで 1.5m / 4m / 6.5m のいずれか。どれが適用されるかは
用途地域と条例で決まります（低層住居専用・田園住居は1.5m、中高層住専と
住居系・近隣商業・準工業は4mまたは6.5m）。
`ShadowRegulationSpec.measurement_height_m` で指定します。

> **未修正** — 用途地域の指定のない区域（別表第四 四の項）は
> イ = 1.5m / ロ = 4m の2択で、**6.5m はありません**。`zoning.py` の
> `ALLOWED_MEASUREMENT_HEIGHTS_M` は 6.5m を許しており、これは危険側の
> 誤りです（`docs/mvce/legal_basis.md` の「食い違い R」）。

ここでいう「平均地盤面」は別表第四末尾の定義によるもので、
**令2条2項と違い「高低差3m以内ごと」の区分がありません**。敷地全体で
1つです（同「食い違い T」）。

...
`solvers/optimizer.py` の既定（ボクセル貪欲法）は、超過している測定点ごとに
...
どちらも、`index/shadow_index.py` の「しきい値高さ」インデックスをそのまま使い、
...
（`tests/mvce/test_shadow_envelope.py`）。天空率で行った検証と同じ考え方です。

## 天空率との同時最適化

`sky_index` を渡すと、棟高の二分探索が**日影と天空率の両方**を条件にします
（`solvers/optimizer.py` の従来の実装は、屋根形状で日影を解消したあとに天空率だけを
...
是正は呼び出し側（`solvers/optimizer.py`）が別途フリーフォームで行います。
"""
...
from ..index.sky_index import SkyIndex
